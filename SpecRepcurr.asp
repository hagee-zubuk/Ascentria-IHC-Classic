...
ElseIf Request("SelRep") = 67 Then 'newsletter
		...
		ElseIf Request("SelRep") = 69 Then 'medicaid consumer record
			dim myDay(6), tbl(6), myUnits(6), myAC(6)
			...
			myDay(6) = "sat"
			'1st week			
			sqlTBL = "SELECT distinct client, emp_ID, lname, fname, address, city, state, zip FROM Tsheets_T, Consumer_T WHERE client = medicaid_number AND code = 'M' AND " & _
				"date = #" & CDate(Request("closedate")) & "# AND EXT = False AND NOT ProcMed IS NULL ORDER BY lname, fname"
			'response.write sqltbl
			Set rsTBL = Server.CreateObject("ADODB.RecordSet")
			rsTBL.Open sqlTBL, g_strCONN, 3, 1
			Do Until rsTBL.EOF
				'myhrstot = rsTBL("sun") + rsTBL("mon") + rsTBL("tue") + rsTBL("wed") + rsTBL("thu") + rsTBL("fri") + rsTBL("sat") 
				'if myhrstot > 0 Then
					myTotHours = 0
					strBODY2 = "<table style='border: 1px solid black;'><tr><td align='left' colspan='4'><b>Medicaid Consumer Record</b></td>" & vbCrLf & _
							"<td style='background: url(images/printlogo.gif) no-repeat top left;' colspan='4' rowspan='4'><img src='images/printlogo.gif' border='0'</td></tr>" & vbCrLf & _
							...
							"<tr><td>&nbsp;</td><td align='center'><font size='1' face='trebuchet ms'>" & Request("closedate") & "</font></td><td align='center'><font size='1' face='trebuchet ms'>" & DateAdd("d", 1, Request("closedate")) & "</font></td>" & _
							"<td align='center'><font size='1' face='trebuchet ms'>" & DateAdd("d", 2, Request("closedate")) & "</font></td><td align='center'><font size='1' face='trebuchet ms'>" & DateAdd("d", 3, Request("closedate")) & "</font></td>" & _
							"<td align='center'><font size='1' face='trebuchet ms'>" & DateAdd("d", 4, Request("closedate")) & "</font></td><td align='center'><font size='1' face='trebuchet ms'>" & DateAdd("d", 5, Request("closedate")) & "</font></td>" & _
							"<td align='center'><font size='1' face='trebuchet ms'>" & DateAdd("d", 6, Request("closedate")) & "</font></td></tr>" & vbCrLf & _
							"</tr>" & vbCrLf 
					sqlTBLWor = "SELECT distinct emp_ID, lname, fname FROM Tsheets_T, worker_T WHERE emp_ID = Social_Security_Number AND client = '" & rsTBL("client") & "' " & _
						"AND date = #" & CDate(Request("closedate")) & "# AND EXT = False AND NOT ProcMed IS NULL ORDER BY lname, fname, emp_ID" 	
					Set rsTBLWor = Server.CreateObject("ADODB.RecordSet")
					rsTBLWor.Open sqlTBLWor, g_strCONN, 3, 1
					Do Until rsTBLWor.EOF
						for lngI = 0 to 6
							sql1 = "SELECT emp_id, " & myDay(lngI) & " AS [val], misc_notes FROM tsheets_t WHERE client= '" & rsTBL("client") & "' AND date = #" & CDate(Request("closedate")) & "# and " & myDay(lngI) & " > 0 AND EXT = False AND NOT ProcMed IS NULL ORDER BY timestamp"
							Set tbl(lngI) = Server.CreateObject("ADODB.Recordset")
							tbl(lngI).Open sql1, g_strCONN, 3, 1
						Next
						do while not (tbl(0).eof and tbl(2).eof and tbl(3).eof and tbl(4).eof and tbl(5).eof and tbl(6).eof)
							myhrstot = 0
							strBODY2 = strBODY2 & "<tr><td colspan='8'>&nbsp;</td></tr><tr><td align='left' colspan='8'><font size='1' face='trebuchet ms'><b>" & GetNameWork(rsTBLWor("emp_ID")) & "</b></font></td></tr>" & vbCrLf
							strBODY2 = strBODY2 & "<tr><td style='border: 1px solid black; text-align: right;'><font size='1' face='trebuchet ms'>Hours</font></td>"
							for lngI = 0 to 6
								myAC(lngI) = ""
								If Not tbl(lngI).EOF Then
									...
									Next
								End If
								myHours = Z_FixEOF(tbl(lngI))

								strBODY2 = strBODY2 & "<td align='center' style='border: 1px solid black; text-align: right;'><font size='1' face='trebuchet ms'>" & myHours & "</font></td>"
							...
							for lngI = 0 to 6
									strBODY2 = strBODY2 & "<td align='center' style='border: 1px solid black; text-align: right;'><font size='1' face='trebuchet ms'>" & myUnits(lngI) & "</font></td>"
							next
					...
					strBODY2 = strBODY2 & "<tr><td colspan='8'>&nbsp;</td></tr><tr><td colspan='8'>&nbsp;</td></tr><tr><td><font size='1' face='trebuchet ms'>Total Hours</font></td>" & vbCrLf & _
						"<td align='center'><font size='1' face='trebuchet ms'><b>" & myTotHours & "</b></font></td><td colspan='7'>&nbsp;</td></tr><tr><td><font size='1' face='trebuchet ms'>Total Units</font></td>" & vbCrLf & _
						"<td align='center'><font size='1' face='trebuchet ms'><b>" & myTotHours * 4 & "</b></font></td><td colspan='7'>&nbsp;</td></tr>" & vbCrLf & _
...
<html>
	<head>
		...
		function hidetxt()
		{
			if (document.frmRep.SelRep.value == 18 || document.frmRep.SelRep.value == 24 || document.frmRep.SelRep.value == 59 || document.frmRep.SelRep.value == 60 || document.frmRep.SelRep.value == 29 || document.frmRep.SelRep.value == 32 || document.frmRep.SelRep.value == 31 || document.frmRep.SelRep.value == 41 || document.frmRep.SelRep.value == 47 || document.frmRep.SelRep.value == 49 || document.frmRep.SelRep.value == 50 || document.frmRep.SelRep.value == 52 || document.frmRep.SelRep.value == 66)
				{
		...
		function hided8()
		{
			if (document.frmRep.SelRep.value == 27 || document.frmRep.SelRep.value == 39 || document.frmRep.SelRep.value == 44  || document.frmRep.SelRep.value == 55 || document.frmRep.SelRep.value == 51 || document.frmRep.SelRep.value == 64 || document.frmRep.SelRep.value == 69)
			...
			function weeknum()
			{
				document.frmRep.action = "weeknum.asp?uri=" + document.frmRep.selUri.value + "&opt=" + document.frmRep.selopt.value + "&tmpdate=" + document.frmRep.closedate.value;
				document.frmRep.submit();
			}
			function PrintPrev()
			{
