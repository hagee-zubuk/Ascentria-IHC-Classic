...
ElseIf Request("SelRep") = 67 Then 'newsletter
		...
		ElseIf Request("SelRep") = 69 Then 'medicaid consumer record
			...
			Do Until rsTBL.EOF
				...
				response.write sqlTBLWor &  "<br>"
				rsTBLWor.Open sqlTBLWor, g_strCONN, 3, 1
				Do Until rsTBLWor.EOF
					for lngI = 0 to 6
						sql1 = "SELECT emp_id, " & myDay(lngI) & " AS [val], misc_notes FROM tsheets_t WHERE client= '" & rsTBL("client") & "' AND emp_ID= '" & rsTBLWor("emp_ID") & "' AND date = #" & CDate(Request("closedate")) & "# and " & myDay(lngI) & " > 0 AND EXT = False AND NOT ProcMed IS NULL ORDER BY timestamp"
