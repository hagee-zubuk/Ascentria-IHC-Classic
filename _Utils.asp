...
Function GUIDExists(xxx)
	...
	If fso.FileExists(uploadFilePath & xxx & ".PDF") Then GUIDExists = True
	Set fso = Nothing
End Function
...
Function Z_DateNull(var)
    ...
    End If
End Function
Function Z_FormatDate(strDte)
	strYYYY = CStr(DatePart("yyyy", strDte))
   If Len(strMM) = 2 Then strYYYY = "20" & strYYYY
   	
  strMM = CStr(DatePart("m", strDte))
  If Len(strMM) = 1 Then strMM = "0" & strMM

  strDD = CStr(DatePart("d", strDte))
  If Len(strDD) = 1 Then strDD = "0" & strDD

  Z_FormatDate = strMM & "/" & strDD & "/" & strYYYY
End Function
...
Function Z_ListLanguages(lang)
	langsel = ""
	lang = Z_Clng(lang) 
	Set rsLang = Server.CreateObject("ADODB.Recordset")
	rsLang.Open "SELECT [index], [Language] FROM [language_T]  ORDER BY [Language]", g_strCONNDB, 3, 1
	Do While Not rsLang.EOF
		langsel = langsel & "<option value=""" & rsLang("index") & """"
		If (lang = rsLang("index")) Then langsel = langsel & " selected"
		langsel = langsel & ">" & rsLang("language") & "</option>" & vbCrLf
		rsLang.MoveNext
	Loop
	rsLang.Close
	Set rsLang = Nothing
	Z_ListLanguages = langsel
End Function
