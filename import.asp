...
Function GetHrs(strTimeIn, strTimeOut)
	...
	If strMin > 0 Then 
		...
		If Cdbl(tmpBillMMin) >= 0.00 And  Cdbl(tmpBillMMin) < 0.133 Then
			GetHrs = tmpBillMHrs
		ElseIf  Cdbl(tmpBillMMin) >= 0.133 And  Cdbl(tmpBillMMin) < 0.383 Then
			GetHrs = tmpBillMHrs + 0.25
		ElseIf  Cdbl(tmpBillMMin) >= 0.383 And  Cdbl(tmpBillMMin) < 0.633 Then
			GetHrs = tmpBillMHrs + 0.5
		ElseIf  Cdbl(tmpBillMMin) >= 0.633 And  Cdbl(tmpBillMMin) < 0.883 Then
			GetHrs = tmpBillMHrs + 0.75
		ElseIf  Cdbl(tmpBillMMin) >= 0.883 And  Cdbl(tmpBillMMin) < 1 Then
	...
	Else
		...
		If Cdbl(strMin) >= 0.00 And  Cdbl(strMin) < 0.133 Then
			GetHrs = 0
		ElseIf  Cdbl(strMin) >= 0.133 And  Cdbl(strMin) < 0.383 Then
			GetHrs = 0.25
		ElseIf  Cdbl(strMin) >= 0.383 And  Cdbl(strMin) < 0.633 Then
			GetHrs = 0.5
		ElseIf  Cdbl(strMin) >= 0.633 And  Cdbl(strMin) < 0.883 Then
			GetHrs = 0.75
		ElseIf  Cdbl(strMin) >= 0.883 And  Cdbl(strMin) < 1 Then
			GetHrs = 1
		End If
	End If
End Function
...
Function GetWIDBadge(strBadge)
	...
	sqlWID = "SELECT [Social_Security_Number], [index] AS WID FROM Worker_T WHERE Badge = '" & strBadge & "' OR Ubadge = '" & strBadge & "' "
...
Function GetCIDCustID(strCustID)
	...
	Set rsWIDs = Nothing
End Function
Function ConsumerActive(strCustID)
	ConsumerActive = False
	Set rsWIDs = Server.CreateObject("ADODB.RecordSet")
	sqlWID = "SELECT [active] FROM C_Status_t WHERE Medicaid_Number = '" & strCustID & "' "
	rsWIDs.Open sqlWID, g_strCONN, 3, 1
	If Not rsWIDs.EOF Then
		If rsWIDs("active") Then ConsumerActive = True
	End If
	rsWIDs.Close
	Set rsWIDs = Nothing
End Function
...
If Request.ServerVariables("REQUEST_METHOD") = "POST" Then
	...
	oUpload.Files(1).Item(1).Save uploadPath, nFileName
	Set oUpload = Nothing
	'read file
	TimeStamp = Now
	...
	Do Until oCSV.AtEndofStream
		...
		If Trim(strLine) <> "" Then
			...
			Do Until ctr3 = Ubound(csvValue) + 1
				...
				ctr3 = ctr3 + 1
			Loop
			'check activity code if for VA
			If Instr(ActvityCode, "80,") > 0 Or Instr(ActvityCode, "82,") > 0 Then
				If Len(ActvityCode) > 3 Then
					Session("MSG") = Session("MSG") & "ERROR: VA activity code is invalid (LINE: " & ctr & "). Succeeding rows were not saved.<br>"
					Response.Redirect "import.asp"
				End If
			End If
			If ConsumerActive(CID) Then
				If IsWorker(CID, GetWIDSSN(WID)) Or IsFloat(WID) Then
					HrsCon = 0
					tmpMax = 0
					If Instr(ActvityCode, "80,") > 0 Then  'HM vahmhrs
						'Get Total Hours for week 
						Set tblCon = Server.CreateObject("ADODB.RecordSet")
						sqlCon = "SELECT vahmhrs FROM Consumer_t WHERE medicaid_number = '" & CID & "' "
						tblCon.Open sqlCon, g_strCONN, 1, 3
						If Not tblCon.EOF Then
							tmpMax = Z_CZero(tblCon("vahmhrs"))
						End If
						tblCon.Close
						Set tblCon = Nothing
						'GET EXISTING HOURS
						...
						If Not rsChk.EOF Then
							Do Until rsChk.EOF
								If Instr(rsChk("misc_notes"), "80,") > 0 Then
						...
						rsChk.Close
						Set rsChk = Nothing
					ElseIf Instr(ActvityCode, "82,") > 0 Then  'HA vahahrs
						'Get Total Hours for week 
						Set tblCon = Server.CreateObject("ADODB.RecordSet")
						sqlCon = "SELECT vahahrs FROM Consumer_t WHERE medicaid_number = '" & CID & "' "
						tblCon.Open sqlCon, g_strCONN, 1, 3
						If Not tblCon.EOF Then
							tmpMax = Z_CZero(tblCon("vahahrs"))
						End If
						tblCon.Close
						Set tblCon = Nothing
							'GET EXISTING HOURS
						...
						If Not rsChk.EOF Then
							Do Until rsChk.EOF
								If Instr(rsChk("misc_notes"), "82,") > 0 Then
									HrsCon = HrsCon + Z_CZero(rsChk("mon")) + Z_CZero(rsChk("tue")) + Z_CZero(rsChk("wed")) + Z_CZero(rsChk("thu")) + Z_CZero(rsChk("fri")) + _
										Z_CZero(rsChk("sat")) + Z_CZero(rsChk("sun"))
								End If
								rsChk.MoveNext
				...
				End If
			Else
				Session("MSG") = Session("MSG") & "ERROR: Consumer is inactive (LINE: " & ctr & ").<br>"
			End If
		End If
